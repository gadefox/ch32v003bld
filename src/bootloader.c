...
// reset Bootloader Btn Pin
  ...
  if (LOCAL_EXP(GPIO, BOOTLOADER_BTN_PORT)->INDR & (1 << BOOTLOADER_BTN_PIN) && !(RCC->RSTSCKR == 0x10000000))
    boot_usercode();
#else
  if (LOCAL_EXP(GPIO, BOOTLOADER_BTN_PORT)->INDR & (1 << BOOTLOADER_BTN_PIN))
    boot_usercode();
#endif
#else
#if defined(SOFT_REBOOT_TO_BOOTLOADER)
  if ((LOCAL_EXP(GPIO, BOOTLOADER_BTN_PORT)->INDR & (1 << BOOTLOADER_BTN_PIN)) == 0 && !(RCC->RSTSCKR == 0x10000000))
    boot_usercode();
#else
  if ((LOCAL_EXP(GPIO, BOOTLOADER_BTN_PORT)->INDR & (1 << BOOTLOADER_BTN_PIN)) == 0)
    boot_usercode();
#endif
#endif
#endif
...
void usb_pid_handle_in(uint32_t addr, uint8_t *data, uint32_t endp, uint32_t unused, struct rv003usb_internal *ist) {
  ...
  if (endp) { //  TODO: This can be reworked - if it's anything other than "is_descriptor" then send nak.
    // Save some space
    goto sendempty;
  }
  ...
  if (cep == 0) {
    ...
    if (ist->setup_request) {
      ...
      if (s->wRequestTypeLSBRequestMSB == 0x01a1) {
        ...
        if (wlen > sizeof(scratchpad))
          wlen = SCRATCHPAD_SIZE;
